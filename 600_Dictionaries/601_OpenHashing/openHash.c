...
int main(){
    ...
    printf("Welcome to the Chained Hashing Dictionary Test Driver!\n");
...
void insert(Dictionary D, int data){
    ...
    for(trav = &(D)[idx]; (*trav) != NULL && (*trav)->data != data; trav = &(*trav)->link);
...
void print(Dictionary D){
    for(int i = 0; i < MAX; i++){
        ...
        for(nodeType curr = D[i]; curr != NULL; curr = curr->link){
            printf("%d ", curr->data);
        }
